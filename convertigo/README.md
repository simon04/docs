...
It is very likely that `convertigo` does not support the currently selected architecture (`windows-amd64`).
...
```console
$ docker run -d --name C8O-MBAAS -e JAVA_OPTS="-Xmx4096m -DjvmRoute=server1" -p 28080:28080 winamd64/convertigo
```
